...
const dropdownData = [
  ...
  // Material types
  { category: 'material_type', value: 'other', display_order: 1, is_active: true },
  { category: 'material_type', value: 'color', display_order: 2, is_active: true },
...
const seedDropdowns = async () => {
  try {
    console.log('🌱 Starting dropdown seed...');
    ...
    const dataWithIds = dropdownData.map(item => ({
      ...item,
      id: generateId('OPT'),
      is_active: true,
      created_at: new Date(),
    ...
    console.log('✅ Dropdown seed completed successfully!');
    console.log(`\n📊 Summary:`);
    console.log(`   - Total options: ${dataWithIds.length}`);
    console.log(`   - Product categories: ${dropdownData.filter(d => ['weight', 'length', 'width', 'thickness', 'category', 'color', 'pattern', 'unit'].includes(d.category)).length}`);
    console.log(`   - Material categories: ${dropdownData.filter(d => ['material_category', 'material_unit'].includes(d.category)).length}`);
    console.log(`   - Production categories: ${dropdownData.filter(d => ['priority', 'quality_rating', 'waste_type'].includes(d.category)).length}`);
